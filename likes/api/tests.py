...
class LikeApiTests(TestCase):
    ...
    def test_likes_count_with_cache(self):
        ...
        for i in range(3):
            _, client = self.create_user_and_client(
                f"someone{i}", email=f"{i}@example.com"
            )
            client.post(LIKE_BASE_URL, data)

            # check tweet detail api
